...
void Bankaccount::open() {
    ...
    // TESTE 12: Lançar erro se tentar abrir uma conta já aberta
    if (_is_open) {
        throw std::runtime_error("A conta já está aberta");
    }
    _is_open = true;
    _balance = 0;
}
...
void Bankaccount::close() {
    std::lock_guard<std::mutex> lock(mtx);
    // TESTE 11: Lançar erro se tentar fechar uma conta que não está aberta
    if (!_is_open) {
        throw std::runtime_error("A conta não pode ser fechada porque não está aberta");
    }
    _is_open = false;
}
